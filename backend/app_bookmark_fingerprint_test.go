...
func TestBookmarkListExcludesLegacyFingerprintCheck(t *testing.T) {
	...
	for _, item := range list {
		if item.URL == fingerprintCheckBookmarkURL {
			t.Fatalf("legacy fingerprint bookmark remains: %#v", list)
		}
	}
}

func TestBookmarkSaveDoesNotRestoreFingerprintCheck(t *testing.T) {
	app := NewApp(t.TempDir())
	app.config = &config.Config{}
	app.browserMgr = browser.NewManager(app.config, app.appRoot)
...
func TestBookmarkSaveFiltersFingerprintCheck(t *testing.T) {
	app := NewApp(t.TempDir())
	app.config = &config.Config{}
	app.browserMgr = browser.NewManager(app.config, app.appRoot)
