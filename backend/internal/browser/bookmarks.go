...
// RemoveBookmarkURL removes an obsolete managed bookmark from every bookmark folder.
func RemoveBookmarkURL(userDataDir string, targetURL string) (bool, error) {
	...
	for _, item := range roots {
		...
		if children, ok := folder["children"].([]interface{}); ok {
			filtered, removed := removeBookmarkURLFromNodes(children, targetURL)
			if removed {
	...
	return true, os.WriteFile(bookmarksPath, out, 0o644)
}

func RemoveBookmarkName(userDataDir string, targetName string) (bool, error) {
	...
	for _, item := range roots {
		...
		if children, ok := folder["children"].([]interface{}); ok {
			filtered, removed := removeBookmarkNameFromNodes(children, targetName)
			if removed {
				folder["children"] = filtered
				folder["date_modified"] = toChromiumTime(time.Now())
				changed = true
...
func removeBookmarkURLFromNodes(nodes []interface{}, targetURL string) ([]interface{}, bool) {
	...
	for _, item := range nodes {
		...
		if node["type"] == "url" && strings.EqualFold(strings.TrimSpace(fmt.Sprint(node["url"])), targetURL) {
			changed = true
			continue
		}
...
func removeBookmarkNameFromNodes(nodes []interface{}, targetName string) ([]interface{}, bool) {
	result := make([]interface{}, 0, len(nodes))
	changed := false
	for _, item := range nodes {
		node, ok := item.(map[string]interface{})
		...
		if node["type"] == "url" && strings.EqualFold(strings.TrimSpace(fmt.Sprint(node["name"])), targetName) {
			changed = true
			continue
		}
		if children, ok := node["children"].([]interface{}); ok {
