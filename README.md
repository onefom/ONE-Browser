...
1.8.15 新增关闭到托盘与全局 Esc 返回，云备份改为首页按钮下拉面板，并统一代理、日志和侧栏的稳定布局。
...
运行产生的数据统一保存在程序旁的 `data` 目录。1.8.15 空白便携包不会再自动导入 Windows `%APPDATA%` 中的旧窗口；升级时请复制旧版的整个 `data` 文件夹。系统日志页仅显示当前进程会话，磁盘日志位于 `data/logs/app.log`。上传公开仓库前，请确认 `.gitignore` 未被删除。
