...
const stage10Source = await readFile(new URL('../public/one-browser/stage10-network-filter-latency.css', import.meta.url), 'utf8');
const stage11Source = await readFile(new URL('../public/one-browser/stage11-tray-backup-scroll.css', import.meta.url), 'utf8');
...
assert.equal(htmlSource.includes('<div class="header-version" data-header-page="settings" hidden>当前版本 <span>1.8.15</span></div>'), true, '版本号必须以纯文字显示在系统设置页头');
...
assert.equal(appSource.includes("desktop.getWindowNetworkInfo(index)"), true, '网络详情必须按当前窗口实时同步');
assert.equal(appSource.includes("showConfigDetails([['出口 IP',info.ip||'—'],['位置'"), true, '网络详情只保留出口 IP 和位置');
assert.equal(htmlSource.includes('id="advancedFilterPopover"'), true, '首页必须提供高级筛选面板');
assert.equal(htmlSource.includes('id="advancedFilterBtn" aria-expanded="false">筛选</button>'), true, '筛选按钮必须移除向下箭头');
assert.equal(appSource.includes("advancedFilters.kernel") && appSource.includes("advancedFilters.group") && appSource.includes("advancedFilters.proxy"), true, '高级筛选必须支持内核、分组和代理情况');
assert.equal(appSource.includes('setInterval(()=>{if(document.querySelector(\'#proxy\')?.classList.contains(\'active\'))refreshProxyLatency(false)},60000)'), true, '节点延迟必须每 60 秒后台刷新');
assert.equal(appSource.includes('使用此节点'), false, '节点卡片点击即选中，不应保留使用按钮');
assert.equal(appSource.includes("['Timeout','timeout']"), true, '节点测速失败必须显示 Timeout');
assert.equal(stage10Source.includes('width:27px!important') && stage10Source.includes('height:27px!important'), true, '代理删除按钮需要整体缩小 10%');
assert.equal(stage10Source.includes('.proxy-dialog::-webkit-scrollbar,.proxy-pane::-webkit-scrollbar{display:none}'), true, '代理导入弹窗必须隐藏滚动条');
assert.equal(stage10Source.includes('width:min(810px,88vw)!important') && stage10Source.includes('height:min(810px,81vh)!important'), true, '代理导入弹窗必须缩小并保持固定尺寸');
assert.equal(stage10Source.includes('.help-content{min-height:0;overflow-y:auto!important'), true, '帮助中心只能滚动右侧内容区');
assert.equal(stage10Source.includes('.help-content::-webkit-scrollbar{display:none!important}'), true, '帮助中心不能显示可视滚动条');
...
assert.equal(shellSource.includes('runtime?.WindowHide?.()'), true, '关闭主窗口必须隐藏到托盘');
assert.equal(shellSource.includes('await call("ForceQuit")'), false, '关闭主窗口不能直接退出应用');
assert.equal(htmlSource.includes('id="cloudBackupModal"'), false, '云备份不能再使用独立全屏弹窗');
assert.equal(htmlSource.includes('id="cloudBackupPopover"'), true, '云备份必须显示为首页按钮下方的浮层');
assert.equal(appSource.includes("event.key!=='Escape'"), true, '所有临时弹窗必须支持 Esc 返回');
assert.equal(stage11Source.includes('#proxy.page.active,#logs.page.active'), true, '代理和日志页必须固定上方区域');
assert.equal(stage11Source.includes('#logs #systemLogRows::-webkit-scrollbar{display:none}'), true, '系统日志内容滚动条必须隐藏');
assert.equal(stage11Source.includes('width:28px!important;height:28px!important'), true, '侧栏头像展开和折叠必须保持 28px');
